...
@pytest.mark.asyncio
async def test_get_unique_books_by_city(client: AsyncClient, session: Session):
    user_payload = {
        "first_name": "Test",
        "last_name": "User",
        "email": "loltotallytest@girl.yes",
    ...
    library_moscow = models.Library(
            id=1, name="Central Library", city="Moscow",
            address="123 Test St", phone="1234567890", email="moscow_lib@la.la",
            description="Main library for testing", open_at="09:00", close_at="17:00", days_open="Mon-Fri"
    )
    library_inno = models.Library(
            id=2, name="Central Library", city="Innopolis",
            address="123 Test St", phone="1234567890", email="inno_lib@la.la",
            description="Main library for testing", open_at="09:00", close_at="17:00", days_open="Mon-Fri"
    )
    session.add(library_moscow)
    session.add(library_inno)
    session.commit()
    ...
    response = await client.get("/books/unique/city/", headers=headers)
    assert response.status_code == 200
    data = response.json()
    for book in data:
        library = (await client.get(f"libraries/{book["library_id"]}")).json()
        assert library["city"] == "Moscow"
    ...
    await client.delete("/libraries/2", headers=headers)
    assert (await client.get("/libraries/2")).status_code == 404
